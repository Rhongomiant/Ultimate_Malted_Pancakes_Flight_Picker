...
- `Ultimate_Malted_Pancakes_Flight_Picker_v3.0.0-rc.2.html` — the picker app (current)
...
`X.Y.Z` — `Z` bumps **only** for bug fixes or items previously shipped buggy/incomplete; `Y` bumps for layout changes, UX restructures, new features, or anything user-visibly different; `X` is major. When a release bundles both, the higher bump wins. Current: **v3.0.0-rc.2** (picker consolidation + Setup phase reorg, addressing five reviewer items from rc). Picker UI: dry-mix-source pills, Batch Quantity stepper, and the "Allow same flavor in multiple batches" toggle are consolidated into a single `.batch-combined-row` inside the Batch card under small uppercase labels — the multi-batch toggle moves out of the Flavor card entirely. The count stepper is renamed from "Make this batch" to "Batch Quantity" and is fully disabled unless `state.allowMultiBatchFlavor` is true; toggling multi-flavor OFF forces all `batch.count` back to 1 so the data model matches the UI affordance (with `syncActiveBatchToLegacyState` doing the same normalization on every Flight-mode render to catch URL/localStorage loads). New Reset button after the card-scale slider in the controls row: `confirm()`-gated, clears all content state and content `pancakeFlight*` localStorage keys (mode, size, batch, flavor, selectedFlavors, orderMode/sub-order, all four layout settings, both candy preps, bulk, batches, activeBatchIdx, activeCategories, allowMultiBatchFlavor, shoppingListExpanded), clears URL state via `history.replaceState`, and resets in-memory state to initial defaults. Display preferences (theme, page layout, card-scale, controls-collapsed state) are preserved. Hover tooltip on the button explains the scope. Setup phase rendering reorganized: `.batches-row` is now a horizontal-scroll flex container (`display: flex; overflow-x: auto`) with a styled scrollbar, replacing the prior grid that shrank columns. Each `.batch-column` has a 320px min-width so it can grow wide based on its internal F&S/Toppings layout without compressing. A `.scroll-hint` paragraph below each `.batches-row` shows "← scroll horizontally to see all batches →" when the row's scrollWidth exceeds clientWidth; the `updateScrollHints()` helper runs after every render via `attachToggleHandlers`. F&S and Toppings layout switchers (side / stack / grid) return to the Setup options band as session-wide controls, shown only when there are 2+ visible flavors total; each batch column's F&S and Toppings sub-card grids honor the chosen `data-subcard-layout` attribute with `repeat(N, minmax(180px, 1fr))` inline grid columns for side-by-side. New `renderBatchColumnHtml(idx, opts)` helper extracted from `renderFlightOutput`, accepts an optional `categoryFilter` to restrict the column's flavor list to one cookGroup — used for Mode B (below) but harmless for Mode A. Setup phase splits into two render modes: **Mode A** (single horizontal-scroll row, current rc behavior) runs under menu/selection order or when only one category has flavors; **Mode B** (category-grouped sections, new) runs under recommended order when 2+ categories have flavors. Mode B emits one `.category-section` per visible category with `{Category} {temp} {blurb}` header, then its own `.batches-row` containing batch columns filtered to that category's flavors. Per the user's decision, each category section repeats the whole batch column (Dry+Wet + filtered F&S + filtered Toppings + filtered Grease) — so a batch with flavors in two categories appears in both sections with Dry+Wet shown each time. Visual cue: each batch column inside a `.category-section` gets a 3px left border in the accent color. Active-category filter still works on top of Mode B (hiding categories hides their entire section). CSS additions (~150 lines): `.batch-combined-row` + `.bcr-group` + `.bcr-label` with dotted-underline help-cursor on tooltip labels; `.reset-btn` with warn-themed hover; horizontal-scroll `.batches-row` with custom thin scrollbar styling and accent thumb on hover; `.scroll-hint` with arrow accents; `.category-section` with accent top-border, header (h3 + temp badge + blurb), and per-section accent-left batch columns; `.setup-layout-toggles` band styling; `.batch-count-stepper.disabled` opacity rule. Print rules hide the reset button and scroll hints and unstack `.batches-row` into block flow. rc.2 test suite 38/38 passing: stepper disabled state, count normalization on multi-toggle OFF, sync helper normalization, full reset (state + content keys + URL), reset declined via confirm() is a no-op, theme/layout preserved across reset, Mode B 3 category-section count with batch repetition (b1 in 2 cats, b2 in 2 cats, b3 in 1 cat), category section header content, Mode A under single-category, Mode A under menu order with multi-category, F&S/Toppings layout switcher rendering with active states and subcards-grid data-attributes, layout switchers hidden when only 1 visible flavor, picker structural visibility (batch-quantity + multi-batch groups shown in Flight, hidden in Single), updateScrollHints function exists. Prior suites still green: alpha.2 plumbing 10/10, beta picker UI 39/39 (two tests updated to set `allowMultiBatchFlavor = true` before testing `count > 1`), rc rendering 50/50 (data-batch-count attribute removed, multi-category 3-batch test rewritten as Mode B assertions, "filter Clean only" rewritten to verify Mode A activation when only one category is visible). Previous: **v3.0.0-rc** (output rendering reorganization — the biggest single visual change in the v3.0.0 cycle). Replaces the Flight-mode output renderer entirely so the recipe layout consumes `state.batches[]` directly. Setup phase is per-batch columns in a grid (rc) — replaced by horizontal scroll in rc.2. Each column contains a per-batch header, Dry+Wet bowl pair, Fat & Spices sub-cards, Toppings sub-cards, and grease lines. Active-category multi-select pill row in the Setup options band with `[All]` / `[Clear]` quick actions. Cook phase becomes universal across all visible flavor tuples with category-or-flat layout; Quality Tier section moves out of per-batch columns into its own block after Cook phase with batch-source labels on each tier card. Empty/partial batches show placeholder cards. Master shopping list placeholder. Previous: **v3.0.0-beta** (picker UI for the batches model — batch-tabs strip, count stepper, multi-batch toggle, flavor-uniqueness block-with-message, auto-create-empty-batch on Flight entry). Previous: **v3.0.0-alpha.2** (data-model plumbing — batches-as-first-class state shape, helpers, URL encoding, localStorage keys, legacy URL import). Previous: **v2.10.1** (alignment fix on the controls row). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs — shipped) → rc (output rendering reorg — shipped) → final (master shopping list math). The companion manual update remains queued for a dedicated next session. Previous: **v3.0.0-alpha.2** (data-model plumbing for the v3.0.0 batches pivot — no visible UI changes vs. v2.10.1). Replaces the discarded v3.0.0-alpha (per-flavor override model) with batches-as-first-class. New state fields: `batches[]` (array of `{ id, batch: 0-3|null, count: int≥1, flavors[] }`), `activeBatchIdx`, `activeCategories[]` (cook-group filter under recommended order), `allowMultiBatchFlavor`, `shoppingListExpanded`. Adds `FLAVOR_PREP_SCOPE` forward-looking hook keyed by flavor id — all candies default to `'session'`; future time-sensitive candies can set `'batch'` to opt into per-batch prep. Eight helpers: `getPrepScope`, `getActiveBatch`, `addBatch`, `removeBatch`, `getAllFlavorBatchTuples`, `findFlavorInBatches`, `getVisibleBatchIndices`. URL encoding uses repeated `b=batchCode:flavor1,flavor2[:count]` params, plus `activebatch`, `activecat`, `multiflavor`, `shopopen`; legacy v2.10.1 URLs imported as a single synthesized batch. Five new localStorage keys. Alpha.2 checklist 51/51. Previous: **v2.10.1** (alignment fix: each switcher had a stale `margin-bottom: 24px` from before they were grouped in a flex row, which made Hide Options/Controls sit visually lower than the switcher pills and added ~40px of empty space below the row; zeroed those margins and reduced `.controls-row` margin-bottom 16→8px). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs — shipped) → rc (output rendering reorg into per-batch columns + universal cook phase + per-flavor tier section) → final (master shopping list math). The companion manual update remains queued for a dedicated next session.
