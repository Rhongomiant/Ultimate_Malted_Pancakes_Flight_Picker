...
- `Ultimate_Malted_Pancakes_Flight_Picker_v3.0.0-rc.html` — the picker app (current)
...
`X.Y.Z` — `Z` bumps **only** for bug fixes or items previously shipped buggy/incomplete; `Y` bumps for layout changes, UX restructures, new features, or anything user-visibly different; `X` is major. When a release bundles both, the higher bump wins. Current: **v3.0.0-rc** (output rendering reorganization — the biggest single visual change in the v3.0.0 cycle). Replaces the Flight-mode output renderer entirely so the recipe layout consumes `state.batches[]` directly instead of going through beta's `syncActiveBatchToLegacyState` bridge. Setup phase is now a horizontal grid of per-batch columns (1–6 columns, breakpoints at 1100px down to 2-up and 700px down to stacked single-column for narrow viewports). Each column contains a per-batch header (`Batch N · Size × Count · flavor list`), the v2.10.1 Dry+Wet bowl pair side-by-side (50/50, count multiplier baked into the heading when count > 1), Fat & Spices sub-cards per flavor in that batch's filtered set, Toppings sub-cards, and grease lines for just that batch's cooking groups. Batches that are unconfigured (no batch size and no flavors), or partially configured (size only, or flavors only), render a placeholder card inside their column with a precise hint ("Pick a batch size and at least one flavor", "Pick a batch size for this batch", or "Pick at least one flavor for this batch"). Active-category multi-select pill row sits in the Setup options band under recommended order: `[Clean (n)] [Savory (n)] [Candy (n)]` each toggleable with a check-mark when on, plus `[All]` and `[Clear]` quick-action pills. Pills with zero flavors in that category render disabled. Toggling categories filters Setup (hides batches with no flavors in active cats, hides flavor sub-cards from inactive cats within visible columns), the Cook phase, and the Quality Tier section. The master shopping list is deliberately *not* filtered — it always shows totals across all batches. With all categories cleared, output replaces the whole body with "All categories are hidden. Click All above to restore." Cook phase becomes universal across all visible flavor tuples — the per-active-batch behavior from beta is gone. Shared cooking instructions card sits at the top with the combined flavor list passed through the existing `sharedCookingFlowHtml` helper, then under recommended order each cooking category becomes a `cook-category-card` with `{Category} · {temp}` header, the `{detail}` blurb to its right, the standard category descriptor, and a nested grid of per-flavor delta cards each labeled `{Flavor Label} · B{n}` to mark its batch source. Under menu or selection order, the cook section becomes a flat `flavor-deltas-grid` with each delta card carrying the same `· B{n}` label and showing per-flavor heat. The colder-candy-wins heat rule still aggregates across all visible candy flavors regardless of which batch each lives in, so Caramel-deep in Batch 1 + Toffee-deep in Batch 3 still yields a 325°F Candy group. Quality Tier section moves out of the per-batch columns into its own block after the Cook phase. Deduplication by flavor id: when `allowMultiBatchFlavor` is on and the same flavor appears in multiple batches, it gets a single tier card with all its batch labels concatenated (`Plain · B1 · B3`); otherwise one card per `(flavor, batch)` tuple. Single-flavor case keeps the v2.10.1 single-flavor tier grid layout; multi-flavor case keeps the shared-tier-slots block + per-flavor cards layout with the existing side/stack/grid layout switcher. Empty fallbacks throughout: zero batches → "No batches yet" message; no flavors anywhere → Setup options band + active-category pills (if recommended) but no Cook/Tier/Shopping sections rendered. Master shopping list at the very end is a labeled placeholder card noting "Computed totals across all batches arrive in v3.0.0 final." The legacy v2.10.1 Flight render path (the dry+wet pair, single-flavor flat structure, single-tier-block etc. that beta's `syncActiveBatchToLegacyState` bridge was relying on) is removed entirely — the unreachable code that would have caused duplicate `const` parse errors got stripped. Function `buildOrderToggle(suffix)` got hoisted out of the renderer to file scope so the new render path and any other future callers can use it. Active-category pill clicks are handled via delegation on `#output`: `data-acat-toggle="{group}"` toggles a category in `state.activeCategories`; `data-acat-action="all"` sets all three; `data-acat-action="clear"` empties it. Both trigger a re-render and persist through `persistBatchesState`. CSS additions (~200 lines): `.batches-row` grid with 1–6 column variants and breakpoint rules, `.batch-column` styling with internal `.block` overrides for the smaller scale (12px h4 labels, smaller h5), `.batch-empty-state` placeholder card, `.active-category-pills-row` container with `.active-category-pill` + `.active-category-action` + disabled state, `.no-active-categories` empty-filter message, `.cook-category-card` container (header bar + blurb + nested delta grid), `.fdc-batch-label` styling for the small `· B{n}` annotation on delta-card titles, `.tier-section` wrapper with dashed top-border separator and the muted-uppercase h4 styling. Print rules hide the category pills row and unstack the batches into single-column flow with page-break protection on each `.batch-column`. Render output integrity tests 46/46 passing: 3-batch all-categories rendering (column count + content + cook category cards + per-flavor batch labels + tier section + shopping list), active-category filter at Clean-only (column hidden, cook card hidden, sub-cards hidden, retained flavors visible), empty filter state, menu-order flat list with no category pills and no cook-category-cards, single-mode parity check (no batches-row, no category pills, no batch labels), empty-batch placeholder, partial-batch placeholders for both "size only" and "flavors only", per-category count badges (Clean 2, Savory 1, Candy 1), heat rule across batches (deep candies → 325°F, flash candies → 375°F), and URL round-trip stability with all rc-era state including activecat / multiflavor / shopopen. Beta picker UI 39/39 still passing against this file (no regressions in the picker side), and alpha.2 plumbing sanity 10/10 (FLAVOR_PREP_SCOPE, getPrepScope, addBatch null/empty default, removeBatch last-protection + activeBatchIdx adjustment, findFlavorInBatches lookup, getAllFlavorBatchTuples flattening, getVisibleBatchIndices category filtering). Previous: **v3.0.0-beta** (picker UI for the batches model — visible changes in Flight mode, Single mode unchanged from v2.10.1). Adds a batch-tabs strip at the top of the Batch card (Flight mode only), with each tab showing `Batch N · size · flavor count · ×count` summaries and per-tab close buttons (shown only when 2+ batches exist). A "+ Add batch" pill at the end of the strip creates new empty batches. A count stepper (`−` / `+`) inside the Batch card lets the user set how many copies of the active batch to make (1–99, defaults to 1). The Flavor card gets an "Allow the same flavor in multiple batches" toggle pill below the candy-prep row, persisted across sessions. The flavor-uniqueness rule (one flavor per batch by default) blocks clicks on flavors already used elsewhere with an inline message. Entering Flight mode auto-creates one empty batch when `state.batches` is empty. `syncActiveBatchToLegacyState()` bridges the batches model to v2.10.1's render path; `persistBatchesState()` writes all four new localStorage keys on every render-time sync. Beta test suite 39/39 passing. Previous: **v3.0.0-alpha.2** (data-model plumbing for the v3.0.0 batches pivot — no visible UI changes vs. v2.10.1). Replaces the discarded v3.0.0-alpha (per-flavor override model) with batches-as-first-class. New state fields: `batches[]` (array of `{ id, batch: 0-3|null, count: int≥1, flavors[] }`), `activeBatchIdx`, `activeCategories[]`, `allowMultiBatchFlavor`, `shoppingListExpanded`. Adds `FLAVOR_PREP_SCOPE` forward-looking hook. Eight helpers: `getPrepScope`, `getActiveBatch`, `addBatch`, `removeBatch`, `getAllFlavorBatchTuples`, `findFlavorInBatches`, `getVisibleBatchIndices`. URL encoding uses repeated `b=batchCode:flavor1,flavor2[:count]` params plus `activebatch`, `activecat`, `multiflavor`, `shopopen`; legacy v2.10.1 URLs imported as a single synthesized batch. Five new localStorage keys. Alpha.2 checklist 51/51. Previous: **v2.10.1** (alignment fix: each switcher had a stale `margin-bottom: 24px` from before they were grouped in a flex row, which made Hide Options/Controls sit visually lower than the switcher pills and added ~40px of empty space below the row; zeroed those margins and reduced `.controls-row` margin-bottom 16→8px). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs — shipped) → rc (output rendering reorg — shipped) → final (master shopping list math). The companion manual update remains queued for a dedicated next session. Previous: **v3.0.0-alpha.2** (data-model plumbing for the v3.0.0 batches pivot — no visible UI changes vs. v2.10.1). Replaces the discarded v3.0.0-alpha (per-flavor override model) with batches-as-first-class. New state fields: `batches[]` (array of `{ id, batch: 0-3|null, count: int≥1, flavors[] }`), `activeBatchIdx`, `activeCategories[]` (cook-group filter under recommended order), `allowMultiBatchFlavor`, `shoppingListExpanded`. Adds `FLAVOR_PREP_SCOPE` forward-looking hook keyed by flavor id — all candies default to `'session'`; future time-sensitive candies can set `'batch'` to opt into per-batch prep. Eight helpers: `getPrepScope`, `getActiveBatch`, `addBatch`, `removeBatch`, `getAllFlavorBatchTuples`, `findFlavorInBatches`, `getVisibleBatchIndices`. URL encoding uses repeated `b=batchCode:flavor1,flavor2[:count]` params, plus `activebatch`, `activecat`, `multiflavor`, `shopopen`; legacy v2.10.1 URLs imported as a single synthesized batch. Five new localStorage keys. Alpha.2 checklist 51/51. Previous: **v2.10.1** (alignment fix: each switcher had a stale `margin-bottom: 24px` from before they were grouped in a flex row, which made Hide Options/Controls sit visually lower than the switcher pills and added ~40px of empty space below the row; zeroed those margins and reduced `.controls-row` margin-bottom 16→8px). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs — shipped) → rc (output rendering reorg into per-batch columns + universal cook phase + per-flavor tier section) → final (master shopping list math). The companion manual update remains queued for a dedicated next session.
