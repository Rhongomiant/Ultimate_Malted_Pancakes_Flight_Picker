...
- `Ultimate_Malted_Pancakes_Flight_Picker_v3.0.0-rc.4.html` — the picker app (current)
...
**Current: v3.0.0-rc.4** — four bug fixes from rc.3 review.

(1) **F&S heading**: restored v2.10.1's "(into wet bowl)" / "(into wet bowls), per flavor" wording that rc.2 had simplified away.

(2) **Layout switchers fixed**: the rc-era `.batches-row { display: grid; ... }` CSS block with `data-batch-count` variants was still present, shadowing rc.2's flex base and rc.3's `[data-batch-layout="..."]` variants — practical effect was that rc.3's Group/Batch switchers silently did nothing because the dead grid block came later in source order. The 20-line dead block is now stripped.

(3) **Explicit Mode A/B toggle in Flight card**: replaces the auto-derivation (`isRecommended && multi-cat`). New state field `state.outputGrouping = 'batch' | 'category'` (default: `'category'`), persisted at `pancakeFlightOutputGrouping`, encoded in URL as `grouping=` (only when non-default). Exposed as a fourth `.bcr-group` in the Batch card combined row, labeled "Group output by" with "By Batch" / "By Category" pills and a hover tooltip explaining the difference. Mode B (category sections) and Mode A (single batch row + standalone Cook/Tier sections) can now both run under any orderMode and category count — the user picks.

(4) **Cooking cards no longer huge**: `.flavor-deltas-grid`'s `repeat(auto-fit, minmax(280px, 1fr))` was making cards stretch to fill grid tracks. Switched the max to `360px` so cards stay between 240px (readable minimum) and 360px (sensible maximum); `justify-content: start` left-anchors them. For Mode B's `.cs-cook` cards, additional compact treatment: `minmax(200px, 1fr)`, padding 8px/10px, h6 0.76rem, delta-list items 0.78rem. Same compact treatment applied to `.cs-tier` cards.

**Test totals: 191/191 across 6 suites** — alpha.2 plumbing 10/10 · beta picker UI 39/39 · rc rendering 50/50 · rc.2 picker consolidation 40/40 · rc.3 group/batch layouts 34/34 · rc.4 bug fixes 18/18. Eight assertions updated to set `state.outputGrouping = 'batch'` for tests that specifically exercise Mode A behavior.

**Prior releases**: rc.3 (group/batch layout switchers + per-category Cook+Tier in Mode B), rc.2 (picker consolidation: combined Batch card row, reset button, horizontal scroll, F&S/Toppings layout switchers), rc (output rendering reorg: per-batch columns, active-category pills, universal Cook phase, Quality Tier section), beta (picker UI: batch tabs, count stepper, multi-batch toggle, flavor-uniqueness block-with-message), alpha.2 (data-model plumbing: batches-as-first-class state shape, helpers, URL encoding, localStorage keys, legacy URL import). v3.0.0-final still pending: master shopping list math.

**v2 line**: v2.10.1 (controls-row alignment fix). v2.10.0 (single-row controls). v2.9.0 (candy-combo recipe data + Trace Dusting vocab). v2.8.0 (direct-bind layout switchers, centered controls-row, `.phase-options` bands). Earlier: v2.7.1 → v2.0.0. The v2 roadmap is in `v2_HANDOFF.md`; v3 is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override mental model. The companion manual update remains queued for a dedicated next session.
