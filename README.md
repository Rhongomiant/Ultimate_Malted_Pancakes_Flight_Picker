...
- `Ultimate_Malted_Pancakes_Flight_Picker_v3.0.0-alpha.2.html` — the picker app (current)
...
`X.Y.Z` — `Z` bumps **only** for bug fixes or items previously shipped buggy/incomplete; `Y` bumps for layout changes, UX restructures, new features, or anything user-visibly different; `X` is major. When a release bundles both, the higher bump wins. Current: **v3.0.0-alpha.2** (data-model plumbing for the v3.0.0 batches pivot — **no visible UI changes vs. v2.10.1 yet**). Replaces the discarded v3.0.0-alpha (per-flavor override model) with batches-as-first-class. New state fields: `batches[]` (array of `{ id, batch: 0-3|null, count: int≥1, flavors[] }`), `activeBatchIdx`, `activeCategories[]` (cook-group filter under recommended order), `allowMultiBatchFlavor` (lift the same-flavor-only-in-one-batch rule), `shoppingListExpanded`. Adds `FLAVOR_PREP_SCOPE` forward-looking hook keyed by flavor id — all candies default to `'session'` (matching v2.10.1's session-wide caramel/toffee prep); future time-sensitive candies can set `'batch'` to opt into per-batch prep without rearchitecting. Eight new helpers: `getPrepScope`, `getActiveBatch`, `addBatch`, `removeBatch`, `getAllFlavorBatchTuples`, `findFlavorInBatches`, `getVisibleBatchIndices`. URL encoding uses repeated `b=batchCode:flavor1,flavor2[:count]` params per the user's preference, plus `activebatch`, `activecat`, `multiflavor`, `shopopen`; legacy v2.10.1 `?mode=flight&batch=N&flavors=...` URLs are imported as a single synthesized batch for backward compatibility. Five new localStorage keys (`pancakeFlight{Batches,ActiveBatchIdx,ActiveCategories,AllowMultiBatchFlavor,ShoppingListExpanded}`). Render path unchanged — when `state.batches` is empty (the load default), the v2.10.1 Flight render runs identically. Alpha.2 checklist 51/51 passing: state defaults, prepScope hook, addBatch / removeBatch with active-index adjustment and last-batch protection, tuple flattening, flavor lookup, multi-category visibility filtering, URL round-trip stability, sparse-write rule, legacy URL import, Single mode untouched. The v3.0.0 design locked across this session covers: batch tabs picker UI (beta); side-by-side Dry+Wet bowls per batch column in the setup-phase output, with Cook phase as a universal section containing nested category-then-flavor cards under recommended order or a flat flavor-card list under menu/selection; Quality Tier as a per-flavor-card section after Cook phase; master shopping list at the very end; multi-select active-category filter with All/Clear quick actions under recommended order, filtering Setup + Cook + Tier but not the shopping list; flavor uniqueness blocked-with-message by default with an `allowMultiBatchFlavor` toggle to lift; session-wide candy prep with the `FLAVOR_PREP_SCOPE` hook for future batch-scoped candies. Custom user-defined groupings deferred to v3.1. Previous: **v2.10.1** (alignment fix: each switcher had a stale `margin-bottom: 24px` from before they were grouped in a flex row, which made Hide Options/Controls sit visually lower than the switcher pills and added ~40px of empty space below the row; zeroed those margins and reduced `.controls-row` margin-bottom 16→8px). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** (batches model + master shopping list) is specified in `v3_HANDOFF.md` but the design has evolved past the handoff's per-flavor-override mental model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs + active-category filter) → rc (output rendering reorg into per-batch columns + universal cook phase + per-flavor tier section) → final (master shopping list math). The companion manual update (matching v2.9.0 recipe values + Trace Dusting vocab) remains queued for a dedicated next session.
