...
- `Ultimate_Malted_Pancakes_Flight_Picker_v3.0.0-rc.3.html` — the picker app (current)
...
`X.Y.Z` — `Z` bumps **only** for bug fixes or items previously shipped buggy/incomplete; `Y` bumps for layout changes, UX restructures, new features, or anything user-visibly different; `X` is major. When a release bundles both, the higher bump wins. Current: **v3.0.0-rc.3** (group/batch layout switchers + per-category Cook+Tier interleaving in Mode B). Addresses three reviewer items from rc.2. **(1) Categories side-by-side by default**: in rc.2 the Mode B category sections were vertically stacked; they now live inside a `.groups-row` flex container with `data-group-layout="side"` by default and overflow-x scroll matching the inner batches-row pattern. The accent top-border on each `.category-section` is suppressed in side and grid modes (replaced with a card border + padding) since the visual separation comes from the gap, and kept only when groups stack vertically. (2) **Two new layout switchers** in the Setup options band — Group (side/grid/stack) and Batches (side/grid/stack), both defaulting to side. State fields `groupLayout` and `batchLayout`, persisted via `pancakeFlightGroupLayout` / `pancakeFlightBatchLayout` localStorage keys, encoded in URL as `grouplayout=` / `batchlayout=` (omitted when default), included in reset to defaults and the content-keys list. Group layout controls how `.category-section` elements arrange inside `.groups-row` (Mode B only); batch layout controls how `.batch-column` elements arrange inside any `.batches-row` (Mode A's single row AND Mode B's per-category rows). Side = flex horizontal with overflow-x scroll and 360px/320px min-widths so columns grow with content; grid = `repeat(auto-fit, minmax(380px/280px, 1fr))` 2-column responsive grid; stack = flex column with full-width children. The existing `bindLayoutPills` helper already supports this pattern, so wiring up the two new switchers was two function calls. The scroll hint hides automatically when batch layout is set to grid or stack (since overflow doesn't apply). (3) **Per-category Cook + Tier inside each Mode B `.category-section`**: each category section now contains `.cs-setup` (filtered batch columns, current rc.2 behavior), plus new `.cs-cook` (per-flavor delta cards with batch labels, scoped to this category's flavors, sorted by `recommendedSubOrder`) and `.cs-tier` (deduped flavor tier cards with batch labels — single-flavor case keeps the simple tier grid, multi-flavor case keeps the shared-slots + per-flavor cards layout). The standalone Cook-phase header and Tier-section block at the bottom of the output are SUPPRESSED in Mode B since their content is now distributed across category sections; they still render in Mode A (single category or menu/selection order). Master shopping list still renders once at the very end in both modes. Helper `deltaCardHtmlForTuple` was hoisted from inside the `if (visibleFlavorCount > 0)` block to function-top scope of `renderFlightOutput` so the Mode B per-category Cook composition can call it during Setup-body assembly. CSS additions (~80 lines): `.groups-row` with three layout variants (side flex+overflow, grid auto-fit, stack column), scrollbar styling for side variant, `.category-section` becomes a flex column to keep its inner `cs-*` sections stacked vertically inside each category column, `.batches-row` layout variant overrides (`data-batch-layout="grid"` uses `display:grid` with `repeat(auto-fit, minmax(280px, 1fr))`; `stack` uses `flex-direction: column` with full-width children), `.cs-cook` / `.cs-tier` dashed top-border separators and small uppercase muted section titles. Test totals: alpha.2 plumbing 10/10 · beta picker UI 39/39 · rc rendering 50/50 · rc.2 picker consolidation 40/40 · rc.3 group/batch layouts 34/34 — **173/173 across all v3 phases**. The rc.3 suite covers: state defaults (both 'side'), Mode B `.groups-row` wrapper with three `.category-section` children, three `.cs-setup` + three `.cs-cook` + three `.cs-tier` blocks per Mode B output, no standalone Cook-phase header in Mode B, no `.cook-category-card` in Mode B, master shopping list still rendering, group layout switcher renders with side active by default, batch layout switcher renders with side active by default, group switch to grid applies `data-group-layout="grid"` to `.groups-row`, batch switch to stack applies `data-batch-layout="stack"` to `.batches-row` and hides the scroll hint, Mode A retains no `.groups-row` and keeps the standalone Cook-phase header, URL emits `grouplayout` / `batchlayout` only when non-default and decodes back through parseUrlState round-trip, both keys persist to localStorage on every render-time sync, reset clears both keys and returns state to 'side', per-category Cook content includes batch labels like `Plain · B1` inside the right category. Two rc + one rc.2 test were updated to reflect the new structure: rc's `Cook phase has category cards` assertions became `.cs-cook` count checks (since Mode B no longer has standalone `.cook-category-card`), and rc.2's `switchers hidden when only 1 visible flavor` became `spices/toppings hidden, group/batch always shown`. Previous: **v3.0.0-rc.2** (picker consolidation + Setup phase reorg, addressing five reviewer items from rc). Combined `Dry mix source` / `Batch Quantity` / `Allow same flavor in multiple batches` into a single `.batch-combined-row` inside the Batch card. Renamed `Make this batch` → `Batch Quantity`, fully disabled stepper when `allowMultiBatchFlavor` is off, force-reset all `batch.count` to 1 when toggling off. Reset button after card-scale slider in the controls row. `.batches-row` became horizontal-scroll flex with custom thin scrollbar and `.scroll-hint` below; F&S/Toppings layout switchers returned to the Setup options band. Mode A (single horizontal row) vs Mode B (category sections) split based on `isRecommended && visibleCategories.length > 1`. rc.2 test suite 38/38 passing. Previous: **v3.0.0-rc** (output rendering reorganization). Setup phase per-batch columns (grid layout, now replaced by horizontal scroll in rc.2). Active-category multi-select pills with All/Clear quick actions. Universal Cook phase with cross-batch flavors (in Mode B reorganized to per-category in rc.3). Quality Tier section after Cook phase. Master shopping list placeholder. Previous: **v3.0.0-beta** (picker UI for the batches model — batch-tabs strip, count stepper, multi-batch toggle, flavor-uniqueness block-with-message, auto-create-empty-batch on Flight entry). Previous: **v3.0.0-alpha.2** (data-model plumbing — batches-as-first-class state shape, helpers, URL encoding, localStorage keys, legacy URL import). Previous: **v2.10.1** (alignment fix on the controls row). Previous: **v3.0.0-rc** (output rendering reorganization — the biggest single visual change in the v3.0.0 cycle). Replaces the Flight-mode output renderer entirely so the recipe layout consumes `state.batches[]` directly. Setup phase is per-batch columns in a grid (rc) — replaced by horizontal scroll in rc.2. Each column contains a per-batch header, Dry+Wet bowl pair, Fat & Spices sub-cards, Toppings sub-cards, and grease lines. Active-category multi-select pill row in the Setup options band with `[All]` / `[Clear]` quick actions. Cook phase becomes universal across all visible flavor tuples with category-or-flat layout; Quality Tier section moves out of per-batch columns into its own block after Cook phase with batch-source labels on each tier card. Empty/partial batches show placeholder cards. Master shopping list placeholder. Previous: **v3.0.0-beta** (picker UI for the batches model — batch-tabs strip, count stepper, multi-batch toggle, flavor-uniqueness block-with-message, auto-create-empty-batch on Flight entry). Previous: **v3.0.0-alpha.2** (data-model plumbing — batches-as-first-class state shape, helpers, URL encoding, localStorage keys, legacy URL import). Previous: **v2.10.1** (alignment fix on the controls row). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs — shipped) → rc (output rendering reorg — shipped) → final (master shopping list math). The companion manual update remains queued for a dedicated next session. Previous: **v3.0.0-alpha.2** (data-model plumbing for the v3.0.0 batches pivot — no visible UI changes vs. v2.10.1). Replaces the discarded v3.0.0-alpha (per-flavor override model) with batches-as-first-class. New state fields: `batches[]` (array of `{ id, batch: 0-3|null, count: int≥1, flavors[] }`), `activeBatchIdx`, `activeCategories[]` (cook-group filter under recommended order), `allowMultiBatchFlavor`, `shoppingListExpanded`. Adds `FLAVOR_PREP_SCOPE` forward-looking hook keyed by flavor id — all candies default to `'session'`; future time-sensitive candies can set `'batch'` to opt into per-batch prep. Eight helpers: `getPrepScope`, `getActiveBatch`, `addBatch`, `removeBatch`, `getAllFlavorBatchTuples`, `findFlavorInBatches`, `getVisibleBatchIndices`. URL encoding uses repeated `b=batchCode:flavor1,flavor2[:count]` params, plus `activebatch`, `activecat`, `multiflavor`, `shopopen`; legacy v2.10.1 URLs imported as a single synthesized batch. Five new localStorage keys. Alpha.2 checklist 51/51. Previous: **v2.10.1** (alignment fix: each switcher had a stale `margin-bottom: 24px` from before they were grouped in a flex row, which made Hide Options/Controls sit visually lower than the switcher pills and added ~40px of empty space below the row; zeroed those margins and reduced `.controls-row` margin-bottom 16→8px). **v2.10.0** introduced the single-row controls (actions to the left, page width 1600→1800). **v2.9.0** corrected candy-combo recipe data and added Trace Dusting vocab. **v2.8.0** introduced direct-bind for all layout switchers, centered controls-row, and relocated phase options into dedicated `.phase-options` bands. Earlier releases: v2.7.1 specificity fix, v2.7.0 unconditional layouts, v2.6.0 picker-region wrapper, v2.5.0 stacked width parity, v2.4.0 eight-item polish, v2.3.0 12-item refinement, v2.2.0 10-item layout overhaul, v2.1.1 URL state + sticky chrome, v2.1.0 Flight mode, v2.0.0 card-scale slider. The v2 roadmap is in `v2_HANDOFF.md`; **v3** is specified in `v3_HANDOFF.md` but the design evolved past the handoff's per-flavor-override model — v3.0.0 phases as alpha.2 (data plumbing — shipped) → beta (picker UI with batch tabs — shipped) → rc (output rendering reorg into per-batch columns + universal cook phase + per-flavor tier section) → final (master shopping list math). The companion manual update remains queued for a dedicated next session.
